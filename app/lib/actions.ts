      ...
      errors: validatedFields.error.issues.reduce((acc: any, curr: any) => {
        ...
        if (fieldName === "breaks" && path.length > 1) {
          ...
          for (let i = 0; i < detailsArray.length; i += 2) {
            if (detailsArray[i] === 'breaks' || detailsArray[i + 1] === 'breaks') {
              continue
            }
            const detail = { id: detailsArray[i], fieldName: detailsArray[i + 1]};
            const exists = acc.breaks.errors.some(
              (err: any) => err.id === detail.id && err.fieldName === detail.fieldName
            );
            if (!exists) {
