...
- Wire the Phase 5 design tokens (`design-system/tokens/*.css`) into the app's Tailwind config / global CSS as the theme source, rather than shadcn's stock defaults.
- Re-implement existing UI components (`Button`, `FormControl`, `Tag`, `Clock`, etc. under `app/ui/`) using shadcn primitives styled to match `design-system/components/`.
